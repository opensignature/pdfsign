    ...
    const char *pin, const char *id, const char* md)
{
    ...
    int flags = CMS_CADES | CMS_NOSMIMECAP | CMS_BINARY | CMS_DETACHED | CMS_PARTIAL;

    OpenSSL_add_all_algorithms();
    ERR_load_crypto_strings();

    if ((engine = ENGINE_by_id("pkcs11")) == NULL)
        raise_podofo_error_with_opensslerror( "Failed to load ENGINE" );

    if (!ENGINE_set_default(engine, -1))
        raise_podofo_error_with_opensslerror( "Failed to set ENGINE" );
    ...
    sign_md = EVP_get_digestbyname(md != NULL ? md : "sha256");
    ...
    if (si == NULL)
        raise_podofo_error_with_opensslerror( "Failed to add signer" );

    if (!CMS_final(cms, mem, NULL, flags))
        raise_podofo_error_with_opensslerror( "Failed to final CMS" );
...
int main( int argc, char* argv[] )
{
    ...
    pdf_int32 min_signature_size = 8192;
